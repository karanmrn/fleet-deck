...
"  } else {",
"    var cards=[['tokens',fmt(t.totalTokens)],['cost',t.costUsd===null?'unknown':money(t.costUsd)],['sessions',fmt(t.sessions)],['events',fmt(t.events)],['models',String(t.models)],['sources',String(t.sources)]];",
...
"      return '<tr><td>'+esc(m.model)+'</td><td>'+esc(m.provider)+'</td><td class=\"num\">'+fmt(tok(m))+'</td><td class=\"num\">'+money(m.cost_usd)+'</td><td class=\"num\">'+m.sessions+'</td><td class=\"flag\">'+flags.join(' ')+'</td></tr>';",
"    }).join('');",
