...
Tables `totals`, `models`, `days`, `sources` and `energy` carry the same data in camelCase with a `totalTokens` column. In `models`, `flags` joins `estimated`, `partial`, `cost_unknown` and `list_price` with `+`, or is `-`. A cost of `unknown` is not zero. A `listPriceEquivalentUsd` of `-` means no subscription-billed usage, not zero.
