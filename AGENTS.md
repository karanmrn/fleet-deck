...
- Per-provider billing mode: table default in `prices.json` `billing`, machine override in `~/.fleet-deck/config.json` (documented in README "Billing mode"). Subscription-billed costs are exported as `listPriceEquivalentUsd`, never `costUsd`.
